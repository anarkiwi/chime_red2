...
* Channel 10 is dedicated to percussion: instead of notes it plays a small built-in GM-style drum kit (see "Percussion (channel 10)" below). Notes outside the drum map are silent.
...
Channel 10 plays a built-in drum kit rather than tonal notes. Each supported GM
drum note triggers a hardcoded *schedule* that emulates a drum by combining
features the synth already has — a fast pitch drop (kick/tom), a noise burst
(snare/hats/clap), and a percussive amplitude envelope. The schedules are fixed
in firmware (not MIDI-controllable), and each drum is a one-shot: it plays out
and frees its voice on its own, so a note-off is optional.

Supported notes (standard GM drum map):

| Note | Drum | How it's made
| --- | --- | ---
|36|Bass drum (kick)|A fast downward pitch glide from a bass note — the classic synthesised kick.
|38|Snare|A short noise burst with a brief tonal drop underneath.
|42|Closed hi-hat|A brief, bright noise burst with a very fast decay.
|46|Open hi-hat|A bright noise burst that rings out longer.
|45|Low tom|A slower downward pitch glide, tonal (no noise).
|39|Hand clap|A medium noise burst.

Notes on channel 10 that are **not** in this map produce no sound. Drums share
the same 16-oscillator pool as the melodic channels, so a very dense pattern
played alongside many held notes may drop the occasional hit (it is never an
error — the extra hit is simply skipped).
